...
# Compares timings and results to PyMBAR
# (initial comparison by Michael Shirts)
...
for l in xrange(K):
    ...
    for k in xrange(K):
        ...
        u_kln[k, l, N_k[l]:] = np.NaN

# Run UWHAM
startt = time.clock()
results = uwham.UWHAM(u_kln, N_k)
endt = time.clock()
print "uwham %10.3f" % (endt-startt)

#pdb.set_trace()

# MBAR takes a transposed u_kln
u_kln = np.transpose(u_kln, (1, 0, 2))
startt = time.clock()
mbar = pymbar.MBAR(u_kln,N_k,verbose='verbose')
endt = time.clock()
print "mbar %10.3f" % (endt-startt)
u_kln = np.transpose(u_kln, (1, 0, 2))

# Check that f_k values make sense

# UWHAM
ln_N_k = np.log(N_k)
denoms = results._ln_sum_exp(
    (ln_N_k + results.f_k)[:, np.newaxis, np.newaxis] - u_kln, axis=0)
check_f_k = -results._ln_sum_exp(
    np.reshape(-u_kln - denoms[np.newaxis, :, :], (K, -1)), axis=1)

print 'uwham code:'
print ' original f_k = ', results.f_k
print ' result of Eq (11) of MBAR paper = ', check_f_k
print ' difference = ', check_f_k - results.f_k
print ''

# Pymbar
ln_N_k = np.log(N_k)
denoms = results._ln_sum_exp(
    (ln_N_k + mbar.f_k)[:, np.newaxis, np.newaxis] - u_kln, axis=0)
check_f_k = -results._ln_sum_exp(
    np.reshape(-u_kln - denoms[np.newaxis, :, :], (K, -1)), axis=1)

print 'pymbar code:'
print ' original f_k = ', check_f_k
print ' result of Eq (11) of MBAR paper = ', mbar.f_k
print ' difference = ', check_f_k - mbar.f_k
print ''

print 'Difference between uwham and pymbar results = ', results.f_k - mbar.f_k
